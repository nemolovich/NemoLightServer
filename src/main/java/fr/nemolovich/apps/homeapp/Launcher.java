	...
	public static void main(String[] args) throws IOException {
		...
		for (String arg : args) {
			if (arg.equals("--" + ((String[]) PARAMS.get(0))[0])) {
				...
				for (Entry<WebRoute, RouteElementMethod> entry : DeployResourceManager.ROUTES
						.entrySet()) {
					RouteElementMethod method = entry.getValue();
					WebRoute route = entry.getKey();
					if (method.equals(RouteElementMethod.GET)) {
						Spark.get(route);
					} else if (method.equals(RouteElementMethod.POST)) {
						Spark.post(route);
					} else if (method.equals(RouteElementMethod.PUT)) {
						Spark.put(route);
					}
				}
