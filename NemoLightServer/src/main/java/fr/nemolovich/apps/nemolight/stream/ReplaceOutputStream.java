...
/**
 * <p>
 * This kind of stream is used to write a content and
 * replace a sequence by another when this first is found.
 * </p>
 * <p>
 * The replacement is made in the {@link #close() close()}
 * method, if an error occurred during the writing then no
 * replacement is made.
 * </p>
 * <p>
 * The {@link #write(int) write(int)} method writes the byte
 * in a temporary buffer and store the buffer once its size
 * is {@link #ARR_SIZE ARR_SIZE}. Its recommended to use
 * only one of the differents write methods
 * ({@link #write(int) write(int)}, {@link #write(byte[]) write(byte[])}
 * or
 * {@link #write(byte[], int, int) write(byte[], int, int)})
 * and do not mix.
 * </p>
 *
 * @author Nemolovich
...
public class ReplaceOutputStream extends OutputStream {
	...
	/**
	 * Default constructor with stream in which to write
	 * content once replaced.
	 *
	 * @param out {@link OutputStream}: Stream in which to
	 * write.
	 * @param regex {@link String}: Regular expression to
	 * replace.
	 * @param target {@link String}: Replacement of
	...
	private byte[] witeInBuff(byte[] prev, byte[] value)
		...
		if (prev != null) {
			 ...
			 */
			String concat = new String(result).concat(
			...
			while (matcher.find()) {
				 ...
				 */
				result = new byte[replace.getBytes().length];
				value = new byte[concat.getBytes().length];
				System.arraycopy(replace.getBytes(), 0, result, 0,
					replace.getBytes().length);
				System.arraycopy(concat.getBytes(), 0, value, 0,
